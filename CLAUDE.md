...
**(a) `scripts/` and the `engine/**` lint overrides are human-authored and transcribed verbatim, exactly like `shared/src/contract.ts`.** They are the *second* exemption from §4.11's generator rule, and they earn it for a reason that is not the contract's:
