...
**Last updated:** 30 July 2026 *(⚠️ **Ad hoc frontend-follow-up work on `wp-frontend-jul29-followups` (off `dev`), requested directly by the module's own owner — NOT a packet, and the project's "queue empty, nothing tracked" status from 28 Jul is otherwise unchanged.** **(1) Reschedule unified for FIXED and FLEXIBLE**: the two previously separate mechanisms (a candidate-list picker for FLEXIBLE, a direct time-restate for FIXED) are now one `POST /tasks/:id/reschedule` that fully restates the task and writes a new placement wherever the user says, retiring the old occurrence as history (`status: 'SKIPPED'`, DR-06). FIXED still displaces whatever FLEXIBLE occurrence it now overlaps (FR-RSC-02); FLEXIBLE is rejected (409) if the slot no longer fits. A new **"Rescheduled"** panel replaces "Missed," consolidating every retired occurrence with old-time → new-time. **(2)** A completed task can now be undone back to `MISSED` (`RescheduleService.onCompletionUndone`, `POST /tasks/:id/undo-completion`) — deliberately the one way past FR-RSC-07's "never," firing only on the user's own explicit correction. **(3)** A FIXED commitment can no longer be scheduled directly on top of another immovable placement (another FIXED commitment, or any `COMPLETED` occurrence) — FR-CAL-03 says nothing can displace one to resolve a conflict, so overlap is now rejected (409) rather than silently double-booked. **Two bugs found by manual browser testing before commit, both fixed the same session**: `allTasksForDate` matched a task to a day by `intendedDate`/`recurrence` only, never by an actual placement, so a rescheduled task's title rendered as "(task unavailable)" on whichever side of the move wasn't its `intendedDate`; and reschedule left `intendedDate` unchanged, so the ORIGINAL date's automatic sweep (FR-RSC-05) re-placed the task there too, leaving a duplicate "ghost" occurrence — both fixed, the second only because it was driven live in a real browser, not caught by `npm run verify` alone. `npm run verify` **507 green**, all **8** freeze entries intact, engine 100%, all guards green, no frozen file touched. **⚠️ Also removed the wellness view's "Last 7 days" panel at the requirement owner's request — this leaves FR-WEL-04 (Essential, D) unsatisfied.** Flagged at the time; the SRS was updated to say so (v2.38, OPEN-37) even though the panel itself was not restored, per CLAUDE.md §0's rule that a stale document must be caught and said out loud even when fixing it isn't the same session's job. **Committed as two commits** (the reschedule/overlap/undo-completion work, and the wellness panel removal, kept separate as unrelated changes per §8.4) and **pushed to `origin/wp-frontend-jul29-followups`** (new branch, no PR opened). Full account: `docs/TEAM-MEETING.md` decision log, 30 Jul; `docs/SRS-v2.md` v2.38. **⚠️ Continued the same day, UNCOMMITTED as of this note — two more SRS-level corrections plus new ad hoc frontend work, all on the same branch:** an elapsed **FIXED** commitment is now classified `MISSED` in place instead of sitting `PLANNED` forever (SRS → **v2.39**, FR-RSC-01); a reschedule can no longer land back on the exact interval it was just skipped or missed from — a real bug, caught by re-reading a frozen packet 06 fixture (SRS → **v2.40**, §3.8.5 rule 2); an opt-in `ELAPSED_CLASSIFICATION=COMPLETED` env var was added as a genuine, NOT-adopted candidate for relaxing FR-RSC-01 in a future release (**OPEN-38**, new); and the wellness view's workout/meal cards now open the Add Task form pre-filled, a second user-driven scheduling path (new `WorkoutSelectionStore`, explicitly not an SRS requirement). **Both SRS corrections required correcting and re-freezing frozen packet 06 tests** — a team decision per §4.12(b), recorded in `docs/TEAM-MEETING.md` rather than treated as routine. `npm run verify` **507 green**. Full account: §9 below, `docs/TEAM-MEETING.md` decision log, 30 Jul.)*
