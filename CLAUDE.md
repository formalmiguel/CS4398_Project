...
> **24 Jul, on `wp-replay-guards` off `dev`, NOT merged:** the reproducibility work of §4.12 — SRS **v2.21**, the hash-based freeze (`npm run freeze`, `scripts/lib/suite-hash.mjs`, rewritten `guard-tests-frozen.mjs`), new packet **02b**, de-hardcoded packets 04/06/07, and **`docs/REPLAY.md`**. `npm run verify` on that branch: **184 tests green** (that is `dev`'s figure — packets 12/13 and their tests live on Miguel's unmerged branches), both freeze entries verified by digest.

> **24 Jul, on `wp-16a-guards` off `dev`, NOT merged:** packet 16a run — the two (I)-requirement guards `scripts/guard-single-placement.mjs` (FR-RSC-03/NFR-MNT-03) and `scripts/guard-engine-purity.mjs` (FR-SCH-05/CON-04), both wired into `npm run verify` (before `test:coverage`) and CI, **plus two `ci.yml` fixes the packet mandated**: the `guard:tests-frozen` step was missing from CI entirely, and checkout needed `fetch-depth: 0` for its provenance diff. `npm run verify` green: 184 tests unchanged, engine 100%, both freeze entries intact. **One escalation, resolved by the owner:** validating both guards against `wp-13-frontend-dashboard` (where `POST /tasks` calls `findCandidateSlots` directly) fired Check C's vocabulary tripwire on that branch's legitimate `placeTask` transport wrapper — a false positive from two terms added beyond the packet's SRS-derived list; the list was narrowed and both guards now pass on `wp-13`. Full write-up: `docs/P16A-REPORT.md`.

**Documents:** **SRS v2.17** complete and internally consistent — cover page, annotated TOC, formal use cases, UML diagrams, wireframes, ~84 verifiable requirements, traceability matrix, sign-off page. Development method written down (`docs/AGENTIC-TDD-WORKFLOW.md`). Roles assigned. Contract amended against SRS v2.5 before first use.
