...
### §3.8.4 — FR-SCH-10 — and §3.8.8's FR-DSH-06 / UC-03, which govern the SAME creation path and were found to conflict with the first two drafts of this section
...
- **FR-SCH-10.** *(Essential, T)* Where the System places **more than one flexible task** into the same schedulable day, it shall place them in **ascending order of priority** (1 first), each resulting placement becoming a busy interval for every task placed after it. Where two tasks share a priority, the **earlier-created** task shall be placed first, so that the order is total and repeatable.
  ...
  > *Note what this does **not** do: a task **already placed** is not evicted by a higher-priority task created later. That is displacement, and it is **FR-SCH-07** (Conditional).* — this line is already in the SRS under FR-SCH-10, and it is the key to reconciling this requirement with UC-03 below: FR-SCH-10 governs tasks placed **together in one operation**, not a live sequence of individual creations over time.
...
- **UC-03 — Add a Task That Conflicts** *(Essential, §3.4)*: on creation, if the preferred window has no room, **the engine returns up to three ranked alternatives, the System presents them, and the user accepts one** before anything is placed. Contrast **UC-02 — Add a Task That Fits**: when the preferred window *does* have room, the engine finds it and the System places it immediately, with no picker.

  > ⚠️ **MANDATORY — this section was drafted twice before this reading, and both drafts were wrong. Read why before writing this endpoint.**
  >
  > **Draft 1** called `findCandidateSlots` directly from a new `planDay` function. Wrong because it duplicated FR-SCH-10's already-frozen, already-tested busy-set-chaining logic that `RescheduleService.sweepElapsed` already provides — see the superseded text this replaced, kept in `docs/P12-REPORT.md` for the record.
  >
  > **Draft 2** deferred ALL flexible-task placement to `sweepElapsed`, called only from `GET /schedule`, reasoning that a direct per-task engine call at creation would let a low-priority task claim a slot before a higher-priority one created moments later existed to contend for it. **That reasoning was itself wrong** — the FR-SCH-10 quote above says explicitly that an already-placed task is *not* evicted by a later, higher-priority creation; that eviction behavior is FR-SCH-07 and is out of scope. Worse, Draft 2 made **FR-DSH-06 and UC-03 unsatisfiable**: by the time `GET /schedule` sweeps and auto-places a task at rank 1, the other two candidates are gone, and packet 13 has nothing left to build a picker for.
  >
  > **The corrected design, matching UC-02/UC-03 literally**: `POST /tasks` for a `FLEXIBLE` task calls `findCandidateSlots` **directly** — a single task, against the **current** busy set for that date (`PLANNED`/`COMPLETED` placements only, same v2.17 invariant `RescheduleService` uses) and the **current** schedulable day, narrowed to the remainder of the day if `intendedDate` is today and the task's own preferred window has already elapsed (the identical caller-side shaping `RescheduleService.askEngine`'s `remainderOfDay`/`askedAbout` already do for every reschedule trigger — necessarily duplicated here since that file is off limits; without it a task created today for an already-past preferred window would be placed in the past and reclassified `MISSED` on the very next retrieval). Read the winning candidate's `withinPreferredWindow`:
  >   - **`true` (UC-02):** no real conflict. Auto-place immediately; write the `Placement`; return it.
  >   - **`false` (UC-03):** the preferred window had no room. **Write nothing.** Return `result.slots` (up to three, already ranked) to the caller. Mark the task `awaitingChoice` (a repository-internal field, not on the shared `Task` type — see below) so the very next `GET /schedule` does not silently auto-place it out from under the picker: FR-RSC-10 requires `sweepElapsed` on **every** retrieval, and its reattempt branch would otherwise treat "awaiting the user's choice" identically to "just hasn't been swept yet."
  >   - **`placed: false` (FR-SCH-06):** report it; write nothing; do **not** mark `awaitingChoice`. This task remains genuinely eligible for `sweepElapsed`'s reattempt — if the day frees up later, the next `GET /schedule` places it automatically, which is the correct backstop for a task that was never offered a choice because there was nothing to choose from.
  >
  > **`POST /tasks/:id/place`** (new endpoint) is where the user's UC-03 choice is written. Body: `{ date, start, end }`. **Re-validate against the CURRENT busy set before writing** — the offered candidates may be stale by the time the user picks (another commitment could have landed since), and FR-CAL-02's overlap rule is not optional just because a slot was offered a moment ago. Reject with 409 if the chosen interval now overlaps something, doesn't match the task's duration, or falls outside the schedulable day. On success, clear `awaitingChoice`.
  >
  > **OPEN-17 remains closed, and by the same mechanism as before for the parts that still apply**: `TaskRepository.tasksForDate` (excluding `awaitingChoice`) is what `sweepElapsed` reads, and a genuinely-unplaceable task (not a UC-03 one) is still picked up and correctly ordered by the existing, frozen `placement-order.test.ts` machinery once the day frees up. **What changed is narrower than it looks**: a task's *first* attempt at placement is now this file's own direct engine call (UC-02/UC-03); only a task that attempt reported `placed: false` — not "had alternatives" — ever reaches `sweepElapsed`'s reattempt branch for its first placement.
  >
  > **This is still not a second placement function.** `findCandidateSlots` is the only function anywhere that decides where a task goes. What this file now also does is *ask* it directly for the one case that is genuinely its own — a live, single-task, per-request decision at creation — exactly as UC-02 and UC-03's own basic paths describe the System doing ("Engine finds…", "System places…").
...
- **`tasksForDate(userId, date)` — the method OPEN-17 actually turns on.** A task document carries an `intendedDate` (Mongo-only, not on the shared `Task` type — see the FR-SCH-10 note above). Return every task where: **(a)** `recurrence` is absent and `intendedDate === date`, or **(b)** `recurrence.frequency === 'DAILY'` and `intendedDate <= date`, or **(c)** `recurrence.frequency === 'WEEKLY'` and `intendedDate <= date` and `date`'s ISO weekday is in `recurrence.daysOfWeek` — **and, in every case, `awaitingChoice` is not `true`** (a UC-03 task mid-picker is not `sweepElapsed`'s to touch). *(FR-TSK-05's "one placement per matching day" — this is the "matching" test.)* This governs whether `sweepElapsed` ever sees the task as a candidate at all; get it wrong and a task silently never places, silently over-places on days nobody asked for, or silently auto-places out from under a picker the user hasn't acted on.
- **`allTasksForDate(userId, date)`** — the same matching rule, **without** the `awaitingChoice` filter. For `GET /tasks` and `GET /schedule`'s own listing, which must show a task even while it's awaiting a UC-03 choice.
- **`awaitingChoiceTaskIds(userId, date)`** and **`setAwaitingChoice(taskId, awaiting)`** — the flag itself. Set on the UC-03 branch of `POST /tasks`; cleared on a successful `POST /tasks/:id/place`.
...
### FR-SCH-10 / FR-DSH-06 / UC-02 / UC-03 / OPEN-17 — the creation path, corrected a third time

**Two earlier drafts of this section are superseded — see the escalation note under FR-SCH-10/FR-DSH-06 above for why, and `docs/P12-REPORT.md` for the full history.** Do not implement either of the two rejected designs (a `planDay` function calling the engine in a loop; deferring every flexible task's first placement to `sweepElapsed`). The corrected design is stated in full above, under FR-DSH-06/UC-03. Summary of what this packet writes:

- `POST /tasks` (`FLEXIBLE`): calls `findCandidateSlots` directly, once, against the current busy set and the remainder-of-day-narrowed schedulable day. `withinPreferredWindow: true` → auto-place (UC-02). `false` → return candidates, write nothing, mark `awaitingChoice: true` (UC-03/FR-DSH-06). `placed: false` → report, write nothing, leave `awaitingChoice` unset so `sweepElapsed` keeps retrying it (FR-SCH-06/FR-RSC-05).
- `POST /tasks/:id/place`: writes the user's accepted candidate, re-validated against the current busy set; clears `awaitingChoice`.
- `TaskRepository.tasksForDate` (the ratified port `sweepElapsed` reads) **excludes** `awaitingChoice` tasks. `TaskRepository.allTasksForDate` (this packet's own, for `GET /tasks` and `GET /schedule`'s listing) does not — the user must still see a task while its choice is pending. `TaskRepository.awaitingChoiceTaskIds` gives the API a cheap way to tell the frontend which tasks currently need a picker.

**`TaskDocument`'s `intendedDate` field (Mongo-only, not on the shared `Task` type)** is what makes any of this possible: `Task` carries no date, so a one-off task needs *something* to anchor it to the date its creator intended, and a recurring task needs an anchor for "starting from." `tasksForDate`/`allTasksForDate` read it to decide inclusion — see below.

**`sweepElapsed` is still called from `GET /schedule`, and ONLY from there** — never from `POST /tasks` or `POST /tasks/:id/place`. FR-RSC-10 requires it on every retrieval; nothing about the corrected creation path changes that.
...
- **Tasks**: `POST /tasks` (FR-TSK-01/02 — validates the table above, requires an `intendedDate` the task is for; a `FIXED` task is saved with its given start/end directly as its `Placement` and then calls `RescheduleService.onCommitmentAdded(commitment, date)`; a `FLEXIBLE` task follows UC-02/UC-03 above — auto-placed, offered candidates, or reported unplaceable), `POST /tasks/:id/place` (UC-03's accepted choice, re-validated), `GET /tasks/:id`, `GET /tasks?date=` (FR-TSK-03, via `allTasksForDate` + `awaitingChoiceTaskIds`), `PATCH /tasks/:id` (FR-TSK-03 general edit; when `durationMinutes` or `preferredWindow` changes, calls `onTaskEdited` per FR-TSK-04's note above), `DELETE /tasks/:id` (FR-TSK-03; FR-TSK-07's future-only-deletion, preserving completion records).
...
- [ ] **UC-02**: a task whose preferred window has room is auto-placed at creation, with `candidates: null`.
- [ ] **UC-03 / FR-DSH-06**: a task whose preferred window is full returns ranked candidates and writes nothing; `POST /tasks/:id/place` writes the accepted one; a stale/conflicting accept is rejected (409); the very next `GET /schedule` does **not** auto-place a task still awaiting its choice.
- [ ] **FR-SCH-10's no-eviction note**: two flexible tasks created sequentially, lower-priority first, both landing in the same preferred window — the first keeps its placement; the second (even if higher priority) is **not** allowed to evict it.
- [ ] **FR-SCH-10, the batch case**: two tasks genuinely unplaceable at creation (not offered a choice — no room anywhere) both place, in priority order, once the day frees up and one `GET /schedule` sweeps them together — proving the wiring into `sweepElapsed` for the case that's still its job, without re-proving `placement-order.test.ts`'s own frozen assertions. A repository-level test confirms `tasksForDate` includes a never-placed task, excludes an `awaitingChoice` one, and correctly expands a `WEEKLY` recurring task only on its matching weekdays.
