...
- **⛔ MANDATORY — REQUIRED TEST CASE: a displaced or edited occurrence that is still `PLANNED` and cannot be re-placed has its row DELETED (SRS v2.16, E11; FR-RSC-02 note; FR-RSC-05).** This is the one case where *"unplaced = absence of a placement"* is **not free**, and it is the case a suite is most likely to skip — pin it explicitly:
  - **Missed and skipped are already out of `PLANNED`** by the time the engine is asked, so leaving no placement costs nothing. **Displaced and edited are not**: v2.14 (E10) leaves a displaced occurrence **`PLANNED` and moved in place**, and an edited one likewise. So when the engine returns `placed: false` for a **displaced or edited** task, the still-`PLANNED` row **must be removed** — *"the only place in this System where a `Placement` is deleted rather than marked"* (v2.16). The repository does this through **`TaskRepository.deletePlacement`**, whose sole caller this is *(§3.6, v2.16)*.
  - **Assert the deletion, and assert why it matters:** after the failed re-placement the occurrence has **no `PLANNED` row** (so FR-RSC-05 recognises it as unplaced and it **gets the next-day offer**), **not** a stale `PLANNED` row at its now-invalid time. *Leaving the row `PLANNED` is green-but-wrong: the task still **looks placed**, so the offer is silently suppressed and NFR-REL-01 quietly fails — the exact defect v2.16 added `deletePlacement` to prevent, and the exact case a from-scratch replay of this packet omitted, producing a passing suite that missed it.*
  - **Do not confuse this with the missed/skipped path above** — that path asserts *nothing is stored*; this path asserts *a `PLANNED` row is removed*. Both are FR-RSC-05; they differ on whether the failed occurrence was still `PLANNED` when the engine said no.
