...
**(iii) ✅ A source URL appears in the attribution.** `server/src/catalog/attribution.ts` and `data/ATTRIBUTION.md` record where `free-exercise-db` came from (FR-LIB-10, The Unlicense). **A substring match on `https://` fires on the very line that proves provenance.** Strip comments and string bodies before matching, as both existing guards do.
