...
- ✅ **OBSERVED RENDERED IN A BROWSER, 28 Jul — the render is confirmed and the FR-ANL-04/UI-04 (D) demonstration is met.** Driven with headless Chrome (`puppeteer-core` → the installed Chrome) against the running dev servers, with two seeded habits at streak 3 / 75%. Screenshot confirmed: the **Habit consistency** table with both rows ("Read 30 minutes", "Morning run"), each showing "3 days", a filled **completion-rate bar** at ~75%, the **"75%"** and **"(3/4)"** text, the window line "Last 365 days, through 2026-07-28", and the **FR-ANL-03 explainer note** — clean in dark theme, active tab highlighted, no blank frame. The `scheduled === 0` "—" case is covered by the earlier live-payload verification. *(The earlier caveat — "not observed, no browser-automation tool" — is resolved; see the decision-log row, 28 Jul.)*
