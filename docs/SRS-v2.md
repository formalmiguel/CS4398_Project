...
|---|---|---|---|
| **2.41** | **30 July 2026** | **Scheduling Algorithm Lead** | **FR-REC-07 corrected: "whose window has already begun" is now judged against the occurrence's own DATE as well as its minute-of-day.** `RecommendationScheduler.aboveTierWorkout` implemented the exclusion as `placement.start <= clock.nowMinute()` — a bare minute-of-day comparison with no reference to `placement.date`. That answers a different question ("is this earlier in *some* day than the current time of day"), and it was wrong in **both** directions: a workout on a **future** date read as already begun from its start minute onward on the current day, so FR-REC-02's replacement silently refused with *"no workout above <tier> to replace"*; and a workout on a **past** date read as still upcoming whenever the clock had not yet reached its start minute, so a closed day's history could be superseded and rewritten. **This is the same defect shape as OPEN-35**, whose fix gave `RescheduleService` a date-aware `hasElapsed(date, endMinute)`; `RecommendationScheduler` never received the equivalent guard, and no test covered it because every frozen suite applies the recommendation to the clock's own date. Fixed with a private `hasBegun(date, startMinute)` mirroring `hasElapsed`: a date strictly before today has begun unconditionally, a date strictly after today has not begun at all, and the minute comparison survives only for `date === today`. **No requirement's obligation changed** — FR-REC-07's exclusions are the same two exclusions, now evaluated against the right day. *Found by rehearsing the 31 July demonstration against the running server, where applying a recommendation to the next day's schedule 409'd for a workout that had plainly not started. Pinned by three tests in a new unfrozen directory (`server/test/recommendation-future-date/`), importing the frozen packet-17a acceptance harness so the real engine and real `RescheduleService` are exercised; two were watched to FAIL against the pre-fix source, and the third pins the genuinely-begun case that must keep refusing. No frozen file touched; all 8 freeze entries verified unchanged. Full reasoning: `docs/TEAM-MEETING.md` decision log, 30 Jul.* |
| **2.40** | **30 July 2026** | **Frontend & Backend Lead** | **§3.8.5 rule 2 corrected: an occurrence being re-placed for a MISSED or SKIPPED trigger now has its own vacated interval forced busy for that one search — closing a bug where the engine could rank its own just-vacated slot rank 1.** Rule 2 (v2.17) said the occurrence is "never in the busy set for its own re-placement," true of the query but not of the outcome: excluding it left the interval genuinely free, and FR-SCH-03's proximity ranking then had nothing closer to the preferred window than the exact slot the task was just skipped or missed from — so a "reschedule" frequently landed back on the time it was retired from, indistinguishable to the user from nothing happening. `RescheduleService.classifyAndReplace` (the shared path behind FR-RSC-01/08/09) now passes the vacated interval as an extra `busy` entry for that call only; `moveInPlace` (FR-RSC-02 displacement, FR-TSK-04 edit) is unaffected — its row stays `PLANNED` and was never at risk of re-selecting itself. **No requirement's obligation changed** — a rule already stated as "never busy for its own re-placement" is now actually true of the outcome, not just the query. *Found reviewing a frozen packet 06 fixture where the real engine ranked 5:45 PM ahead of 4:00 PM for a Gym session skipped from 5:00–6:00 PM: 5:45–6:00 PM was the last 15 minutes of that same vacated slot. Two frozen tests corrected in a RED-style commit (`single-engine.test.ts`, `triggers.test.ts`) and packet 06 re-frozen, per §4.12(b) — this is a team decision, recorded rather than routine. Full reasoning: `docs/TEAM-MEETING.md` decision log, 30 Jul.* |
| **2.39** | **30 July 2026** | **Frontend & Backend Lead** | **FR-RSC-01 corrected: an elapsed FIXED commitment is now classified `MISSED` in place (no successor), where it was previously left `PLANNED` forever.** The requirement's parenthetical read *"applies to flexible tasks only … no missed classification is made"* — true of **re-placement** (a fixed commitment is immovable, FR-RSC-02, and has nothing to re-place) but the SRS had conflated that with **classification**, so an elapsed fixed occurrence's own status was left `PLANNED` indefinitely, with no way for a user or FR-ANL to tell it apart from one still to come. **Re-placement stays flexible-only; classification no longer does.** `RescheduleService.sweepElapsed` gained the `AUTO_MISSED` outcome (and, only under the new opt-in `ELAPSED_CLASSIFICATION=COMPLETED` env override recorded as **OPEN-38**, `AUTO_COMPLETED`) for this case, producing no successor and no reschedule trigger. Also fixed the same session: the schedule view's "Unplaced" section was filtering by "no live placement" alone, so a fixed commitment now correctly classified `MISSED` briefly rendered there too — narrowed to flexible tasks only, since "no valid slot found" describes an engine failure a fixed commitment (never engine-placed) cannot have. **No requirement's obligation to re-place a fixed commitment changed** — an unstated question about its own status is now answered. *(Full reasoning: `docs/TEAM-MEETING.md` decision log, 30 Jul.)* |
| **2.38** | **30 July 2026** | **Frontend & Backend Lead** | **FR-WEL-04 (Essential, D) marked NOT CURRENTLY SATISFIED, and OPEN-37 opened — the wellness view's seven-day panel that implemented it was removed, and the SRS was not updated at the time.** Raised on `wp-frontend-jul29-followups` after the requirement's own owner directed the panel's removal and, when the conflict was flagged, confirmed removing the UI only, not amending this requirement. §3.8.9's FR-WEL-04 entry now carries an explicit warning rather than silently continuing to assert a capability the build no longer has. This also leaves **OPEN-29's closure record** and **v2.32/v2.34's reasoning** stale — both relied on "the FR-WEL-04 seven-day chart" existing to justify narrowing FR-WER-04's dashboard obligation — flagged in place at OPEN-29 rather than rewritten, per this document's own convention of amending in place with a note (see v2.34). **No requirement demoted or reworded** — resolving OPEN-37 (restore the panel, or formally demote FR-WEL-04) is left to the requirement's owner. *(Full reasoning: `docs/TEAM-MEETING.md` decision log, 30 Jul.)* |
...
>
> **2a. ⚠️ Corrected v2.40 — for a MISSED or SKIPPED re-placement specifically, the vacated occurrence's own interval is nonetheless forced busy for that one search.** Rule 2 above is about the general busy-set *query* (a `MISSED`/`SKIPPED` row is already excluded from it by rule 1, since it no longer occupies time) — but excluding it left the interval genuinely free, and the engine's own FR-SCH-03 proximity ranking then had nothing closer to the task's preferred window than the exact slot it was just vacated from, so it frequently ranked rank 1 right back onto it. **A "reschedule" that lands on the time it was skipped or missed from is indistinguishable to the user from nothing having happened**, which is worse than the rule it was meant to satisfy. `RescheduleService.classifyAndReplace` (FR-RSC-01/08/09's shared path) now passes the vacated interval as one extra `busy` entry for that single call only — `moveInPlace` (displacement, FR-RSC-02; edit, FR-TSK-04) is untouched, since its occurrence is still `PLANNED` and already excluded by rule 1's ordinary logic, and it was never at risk of re-selecting itself. **This is a correction of the rule as originally stated, not a new one**: rule 2's "never in the busy set" was true of the query and false of the outcome, and the distinction was invisible until a fixture happened to rank the vacated tail-end first. *Found reviewing a frozen packet 06 fixture (`single-engine.test.ts`) where the real engine ranked 5:45 PM ahead of 4:00 PM for a Gym session skipped from 5:00–6:00 PM — 5:45–6:00 PM was the last 15 minutes of that same vacated slot. Two frozen tests corrected to assert the interval IS busy and the genuinely-next candidate is taken instead (`single-engine.test.ts`, `triggers.test.ts`); packet 06 re-frozen. Full reasoning and the re-freeze decision: `docs/TEAM-MEETING.md`, 30 July.*
>
...
- **FR-RSC-01.** *(Essential, T)* When a flexible task's window has fully elapsed and it is neither complete nor declared skipped, the System shall classify it **missed** and automatically invoke the engine to place it in the next valid slot remaining that day. *(This classification is **inferred from the absence of a completion**, because the user cannot be relied upon to report a miss — the person who skipped their 8 PM run is the least likely person to open the application and say so. The inference is therefore correct as a default and **wrong in one specific case**: the user did the task and did not mark it. **FR-RSC-09 is the remedy for that case**, and the two requirements shall be read together. **Re-placement** applies to **flexible** tasks only: a fixed commitment is immovable by definition (FR-RSC-02), so there is nothing to re-place and no successor is created. See the v2.39 note below for what an elapsed **fixed** commitment IS classified.)*

  > **⚠️ Corrected v2.39 — an elapsed FIXED commitment IS now classified `MISSED`, in place, with no successor.** Until this revision, FR-RSC-01's parenthetical read *"applies to flexible tasks only … no missed classification is made"*, and an elapsed fixed commitment's placement was left `PLANNED` forever — indistinguishable, to a user or to FR-ANL, from one still to come. **The re-placement half of this requirement remains flexible-only** (a fixed commitment is immovable, FR-RSC-02, so there is genuinely nothing to re-place), but the **classification** half does not depend on movability, and leaving it `PLANNED` was an unstated design accident rather than a considered reading of "applies to flexible tasks only." An elapsed fixed occurrence is now marked `MISSED` in place (or `COMPLETED` in place, only under the `ELAPSED_CLASSIFICATION` env override — see OPEN-38) exactly like a flexible one's original row, but produces **no** successor and **no** reschedule trigger, since there is nowhere else for it to go. *`RescheduleService.sweepElapsed` gained this as its `AUTO_MISSED` outcome, distinct from `RESCHEDULED`. No requirement's obligation to RE-PLACE a fixed commitment changed — only the previously-unstated question of what happens to its own status.* Full reasoning: `docs/TEAM-MEETING.md`, 30 July.
...
- **FR-REC-07.** *(Essential, T)* The System shall not reduce the intensity of a workout already marked complete, and shall not replace one whose window has already begun. A workout that was missed or declared skipped and **re-placed into a window that has not yet begun remains eligible** for adjustment. *(The exclusions are about not rewriting the past and not moving something underway. A re-placed workout is neither.)*
  ...
  > **⚠️ Corrected v2.41 — "has already begun" is judged by the occurrence's DATE as well as its minute-of-day.** The implementation compared `placement.start` to `clock.nowMinute()` alone. A `Minute` is minutes since local midnight and carries no calendar (§4.7), so that comparison silently asked "is this earlier in *some* day than the current time of day" — wrong in both directions. **A future date's workout** read as already begun once the wall clock passed its start minute, so FR-REC-02's replacement refused for a workout that could not possibly have started; **a past date's workout** read as not yet begun whenever the clock had not reached its start minute, so a closed day's placement could be superseded and its history rewritten. Both are now excluded correctly by a date-aware `hasBegun(date, startMinute)`, mirroring `RescheduleService.hasElapsed` — **the same correction OPEN-35 made to the reschedule path, applied to the recommendation path, which had never received it.** *This changes no obligation: the two exclusions are unchanged and are simply evaluated against the right day. It is recorded because the requirement's own wording ("already begun") reads as unambiguous while admitting an implementation that is neither, which is exactly the class of gap §4.8 warns an unenforced (T) requirement becomes.* Full reasoning: `docs/TEAM-MEETING.md`, 30 July.
...
- **FR-WEL-04.** *(Essential, D)* Display sleep score and active calories over at least the previous seven days. ⚠️ **NOT CURRENTLY SATISFIED (v2.38, 30 July 2026, OPEN-37) — the wellness view's seven-day panel was removed** on `wp-frontend-jul29-followups` at the requirement owner's direct request. Flagged at the time; the instruction received back was to remove the UI only, not to amend this requirement. Left stated as Essential rather than silently demoted, so the SRS does not claim a capability the build no longer has by omission. Resolution (restore the panel, or formally demote/reword this requirement and chase every cross-reference) is undecided — see OPEN-37.
...
> ⚠️ **Sixteen issues were resolved as of 28 July 2026; two have reopened or been added since (OPEN-37, OPEN-38) and remain open — this line was not updated when OPEN-37 was raised on 30 July and is corrected now.** Each entry keeps the question **as it was originally raised, struck through, above the answer it received** — a closed issue with no recorded answer is not closed, and it is the reasoning, not the verdict, that stops a settled question being re-argued.
>
...
| **OPEN-29** | ✅ **CLOSED 27 July 2026 — the answer is *presentation*, and FR-WER-04 was narrowed to say so (v2.32).** The wellness view (packet 14b) selects its two chart series by literal metric name; on the wider reading of *"no change to … the dashboard"* a third metric would require editing it and FR-WER-04 would be false. **Ratified reading: a new metric must be ingested, stored, evaluated by its rule and reflected in the recommendation with no change to the four remaining — structural — surfaces.** The dashboard is therefore removed from FR-WER-04's list and from FR-REC-11's verification clause, and packet 16b's extensibility guard does not scan the view layer — **not as an exemption, but because the requirement makes no claim there.** ⚠️ **Closure reasoning amended at v2.34, the outcome unchanged.** This record originally continued *"…but a view is not obliged to display it"*, **which contradicts FR-WEL-01** — the view must render every metric in the set for the current date, generically. **The view layer sits outside FR-WER-04 because that obligation lives in FR-WEL-01 and is verified by demonstration (D)**, not because it does not exist. **The two hard-coded metric names build the FR-WEL-04 seven-day chart — a requirement that names both metrics itself** — while the view's current-date list is already generic and already satisfies FR-WEL-01. **The code was correct as written and no work resulted for its owner**, which is what was communicated to them on 27 July and remains true. *(The original analysis searched for metric names instead of reading the file, and so mistook the FR-WEL-04 chart for the whole view.)* *Rejected: dropping the check while leaving the wording, which would leave the SRS asserting what the build no longer verifies.* Raised 27 July while authoring packet 16b. ⚠️ **STALED 30 July 2026 (v2.38, OPEN-37) — the reasoning above depends on "the FR-WEL-04 seven-day chart" existing, and it no longer does; it was removed on `wp-frontend-jul29-followups`.** This closure's outcome (the dashboard sits outside FR-WER-04's protected surfaces) is unaffected, since that turns on FR-WEL-01, not on the chart — but the sentence *"the two hard-coded metric names build the FR-WEL-04 seven-day chart"* now describes code that does not exist. Left in place per this document's own convention (amend in place with a note, see v2.34) rather than rewritten. | Scheduling Lead *(with Data & Wearable Lead — the view is theirs)* | ~~Before packet 16b merges~~ **done 27 July 2026** |
| **OPEN-38** | ⛔ **OPEN 30 July 2026 — should FR-RSC-01's automatic MISSED classification ever be relaxed to an in-place COMPLETED, as an env-gated `ELAPSED_CLASSIFICATION` option now lets the server do?** Raised on `wp-frontend-jul29-followups` while fixing the FIXED-commitment classification gap (v2.39): the fix also added a same-branch, opt-in server env var, `ELAPSED_CLASSIFICATION` (default `MISSED`, the shipping and only Essential-conformant value), whose alternative `COMPLETED` marks an elapsed occurrence complete in place instead — no successor, no reschedule. **This is a genuine candidate for a future relaxation of FR-RSC-01's inference**, not a dev/demo comparison switch: the case it would help is the same one FR-RSC-09 already exists to correct after the fact (the user did the task and never tapped complete) — the open question is whether the System should ever *default* to the generous reading instead of requiring the user to correct a MISSED after it happens. **Not adopted**: FR-RSC-01 is unchanged and remains Essential as stated (auto-classify MISSED); the env var is off by default and the code path it enables is not exercised by the frozen suites. ⚠️ **§2.7.1/§6 bar all Conditional work until every Essential requirement is verified, and no Conditional work is being undertaken at all as of 26 July** — so this is recorded as a design question for a **future release**, not queued work, and must not be re-raised as something to build now. | Frontend & Backend Lead | — (future release; not before 31 July) |
| **OPEN-37** | ⛔ **OPEN 30 July 2026 — FR-WEL-04 (Essential, D) is not currently satisfied.** The wellness view's seven-day sleep-score/active-calories panel, which was FR-WEL-04's entire implementation, was removed at the requirement owner's direct request (`wp-frontend-jul29-followups`). Flagged when raised; the instruction received back was to remove the UI only, not to amend this requirement or restore it. §3.8.9's FR-WEL-04 entry now carries an explicit warning (v2.38) rather than silently continuing to assert a capability the build no longer has, and OPEN-29's closure record is flagged stale for the same reason. **Not resolved**: either the panel is restored, or FR-WEL-04 is formally demoted/reworded and every cross-reference (Appendix B, OPEN-29, v2.32/v2.34) is re-chased — a decision left to the requirement's owner. | Frontend & Backend Lead | — |
