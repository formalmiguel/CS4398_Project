...
- **FR-DSH-08.** *(Conditional, D)* From the schedule view, allow a collapsible **month-grid navigator** showing which days in the visible month have at least one task, with a per-day indicator distinguishing task type, and allow selecting a day from it to navigate the day view (FR-DSH-01/02). *(Convenience on top of the Essential day view, not part of it — §2.7.1's Core Demonstrable Capability does not depend on it.)*
...
| FR-DSH-08 (Conditional) | Test | `GET /schedule/overview` suite — a recurring task appears on every matching date in range, a one-off task only on its own date, and no `Placement` is written by the request itself |
...
| **OPEN-04** | 🟡 **Partly resolved 27 July 2026.** ✅ **Exercise dataset: `yuhonas/free-exercise-db`, licensed under The Unlicense (SPDX `Unlicense`, public domain) — license read from the repository and recorded, satisfying FR-LIB-10.** ✅ **FR-LIB-04 tier mapping documented and deterministic:** `category == stretching` → LOW; else `category == plyometrics` → HIGH; else by `level` (`beginner`→LOW, `intermediate`→MODERATE, `expert`→HIGH); plus a fixed per-tier **typical-duration** rule for FR-LIB-03 (LOW 20 / MODERATE 30 / HIGH 45 min), the dataset carrying none. *(Full mapping + rejected alternatives: `docs/TEAM-MEETING.md` decision log, 27 July; both are transcribed verbatim into packet 11.)* 🔴 **Still open: author the meal library** with calorie counts (FR-REC-08's target) and dietary flags (FR-LIB-06), calorie distribution satisfying FR-LIB-07 — **hand-authored (§4.2), so no dataset-license question arises** — as part of packet 11. **Meals without calorie values cannot be used** — FR-REC-08 would have nothing to target. | Data & Wearable Lead | ~~20 July 2026~~ meal library with packet 11 |
| **OPEN-28** | **Two `Catalog` interfaces must be reconciled.** Packet 17b left a minimal `findWorkouts(tier, count)` port in `RecommendationScheduler` (deferring to packet 11) so its acceptance suite could run against a double; packet 11 defines the full server-local `Catalog`. When 11b GREEN's real seeded catalog is wired into the recommendation/scheduler path (FR-REC-03/04), 17b's port must be **replaced** by packet 11's interface, not left diverging. Not a freeze blocker — a wiring item surfaced at the packet 11 RED gate (27 July). | Data & Wearable Lead *(with Scheduling Lead — 17b is theirs)* | Before the §6 rehearsals |
